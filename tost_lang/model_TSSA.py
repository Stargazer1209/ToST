...
class Block(nn.Module):
    ...
    def __init__(self, config):
        ...
        if config.TSSA:
            self.attn = CausalSelfAttention_TSSA(config)
        else:
            self.attn = CausalSelfAttention(config)
        self.ln_2 = LayerNorm(config.n_embd, bias=config.bias)
        self.mlp = MLP(config)
        eta = 1e-5
        self.gamma1 = nn.Parameter(eta * torch.ones(config.n_embd), requires_grad=True)
        self.gamma2 = nn.Parameter(eta * torch.ones(config.n_embd), requires_grad=True)
    def forward(self, x):
...
class GPTConfig:
    ...
    TSSA: bool = False # True: use TSSA, False: use original attention
class GPT(nn.Module):
    ...
    def configure_optimizers(self, weight_decay, learning_rate, betas, device_type):
        ...
        keywords = ['temp', 'denom']
        # decay_params = [p for n, p in param_dict.items() if p.dim() >= 2 and not ('temp' in n)]
        # nodecay_params = [p for n, p in param_dict.items() if (p.dim() < 2 or 'temp' in n)]
        decay_params = [p for n, p in param_dict.items() if p.dim() >= 2 and not any([kw in n for kw in keywords])]
        nodecay_params = [p for n, p in param_dict.items() if (p.dim() < 2 or any([kw in n for kw in keywords]))]
        optim_groups = [
